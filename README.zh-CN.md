...
- **你会拿到两个不同问题的分别回答。** 涉及代码时，**运行席**在夹具上真的把东西跑一遍、报告
  实际发生了什么；**逻辑席**从不运行它，只问这个方法回答的是不是该问的那个问题。让同一个审查者
  同时回答这两件事，它会回答容易的那个：它报告脚本干净地跑完了——这是真的——而对「这是不是一个
  错的脚本」只字未提。
...
- **失败是响的。** 空输出、超时、非零退出、格式损坏、身份不符——每一种都是**有名字的独立失败**，
  没有任何一种能变成通过。**包括最难看见的那一种：审查者被杀在半路。** wrapper 在交出控制权
  之前先宣告启动，所以"起来了又死掉"报的是 `LAUNCHED_BUT_NO_VERDICT`，而不是与"根本没起来"
  一模一样的空输出。

**面板只审，不生产。** 生产发生在它之外，面板只检查你提交给它的东西。作者与验证者不会
悄悄变成同一个进程。
...
`light-audit` 就是同一个面板的 `mode: 'quick'`：同样的席位、同样的门、同样的终态，
1 轮而不是 3 轮。**两个深度的门完全一致——三轮拦得住的发现，一轮同样拦得住。**

| | `dual-audit` | `light-audit` |
|---|---|---|
| 深度 | 最多 3 轮，含交叉质证 | 1 轮 |
| 成本 | 约 9 次审查调用 | 约 3 次 |
| 什么时候选它 | 定义、阈值、规则、会喂下游的产物、不可逆操作 | 小分歧，或跑了一次没跑出定论 |

一个带深度档的面板，比它取代的那两套机制**既更简单也更严**。旧的那条便宜道是另一套东西：
一个审查者，**brief 由你写**——于是它读的是你对问题的表述而不是问题本身，正是那条贵的道存在的
理由——而且没有 Claude 侧席位，所以「这代码到底跑不跑得通」是靠读出来的答案。quick 档继承了
第一轮独立读取和运行席，**只放弃交叉质证**。
...
你交进去的是完成的工作和它的原始材料。拿回来的是一个终态、每一席**真正读过**的证据、
所有阻塞项，以及需要人来判的东西。一份真实结果（略作删减）：
...
unresolved_p0:
  - "这道门在要求被删掉之后仍然报绿，因为它只检查两个数字有没有出现；一份保留了两个数字、
     却撤销了要求的改写照样能过，于是 CI 会放行一个把席位留在短默认值上的定义"
...
**它没收敛，而且这是第一句话。** 没有分数、没有百分比、没有「大体上还行」。门要么过了要么没过。

**那条阻塞发现带着损害链**——条件、错误、后果——不是关于风格的意见。**给不出损害链的不算发现**，
协议对两侧都这么说。

**预算写在结果里。** 你能看到它停在 18 次硬顶中的第 3 次，所以「它一直跑到双方同意为止」
这件事不可能发生过。

## 怎么读结果

只有五个终态，其中只有一个表示完整的双席审查通过：

| 终态 | 含义 |
|---|---|
| `CONVERGED` | 两道门都过。**只有这一个可以说成「双席审完通过」。** |
| `CONVERGED_SINGLE_SEAT` | 只有一席读过，且通过。门是同一套，但视角只有一个而不是两个，所以它**小于 `CONVERGED`，绝不可与之等同**。由 `codex_only` 返回。 |
...
（`EMPTY_VERDICT_TEXT`、`LAUNCHED_BUT_NO_VERDICT` 等）和一句给人看的 `why`。
...
- **一次审查要几分钟墙钟，而你的控制器未必给得起。** 如果跑 wrapper 的那一层有更短的上限，
  用 `DUAL_AUDIT_OUTER_BUDGET` 告诉它，它会自己收紧超时并**在你的上限之内响亮失败**；同时把
  命令本身的 timeout 给到你的工具允许的最大值。搞错的后果是审查者被杀在半路——会报
  `LAUNCHED_BUT_NO_VERDICT`、绝不会变成通过，但那次审查还是丢了。**这是最容易被看成"什么都
  没发生"的一种失败。**
- **本版仅支持 Linux。** macOS 与 Windows 尚未支持。已在 Ubuntu 24.04 + Node 22 上验证；
  CI 另外跑 Node 18 与 20。
...
**Codex** 作为独立的一侧参与审查。它只读原始材料、看不到另一侧的结论，因而找出了控制器一侧
**结构上不可能发现**的东西——包括一个残存在 git 历史里的私有标识（本仓库自带的发布前扫描器压根
不查这一类）、裁决文法里的一个洞（能让一条写明的阻断项穿过整个面板没人读）、以及删除路径把文件
清单推导了两次却只比较长度。其中数次，控制器一侧的判断与之相反，而且是错的。

**Claude Code** 作为控制器运行：驱动每一轮面板、派发席位、在轮次之间传递状态。

**两个模型都没有被列为 commit 的 co-author。** 在 git 里署名的含义是"参与编写"，而它们在这里都是被
使用的工具——一个驱动面板，一个审查面板。在一个以"作者与验证者不可塌缩"为前提的项目里，「运行的工具」与「为结果负责的人」之间的界线值得保持可见，而不是被压平成一个贡献者计数。
